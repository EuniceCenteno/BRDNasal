...
#write.table(family.2,"~/Desktop/eunice/MSc/Thesis/Qiime/Samples/OnlySamples/Filtered/Qiime/family.0622.txt",sep=",", row.names = TRUE)
...
#write.table(phylum.2,"~/Desktop/eunice/MSc/Thesis/Qiime/Samples/OnlySamples/Filtered/Qiime/phylum.0622.txt",sep=",", row.names = TRUE)
...
#write.table(genus.2,"~/Desktop/eunice/MSc/Thesis/Qiime/Samples/OnlySamples/Filtered/Qiime/genus.0622.txt",sep=",", row.names = TRUE)


## PHYLUM LEVEL
num_genera <- 135 # we need 100 OTUs in order to get the 25 most abundant Genus
...
DeSeq = read.csv("DESeqResults06.22.21.csv", na.strings = c("","NA"), header=TRUE)
str(DeSeq)
...
#testing differences in Pm, Hs, Mh, Mb
#subsetting for each bacterium
# Pasteurella multocida
Pm <- as.data.frame(NewASVTable[,c(2)])
Pm <- merge(metadata, Pm, by.x = 0, by.y = 0)
str(Pm)
colnames(Pm) <- c("Row.names", "BRD", "Date.Collection", "Counts")
str(Pm)
Pm$prevalence <- ifelse(Pm$Counts < 0, 'No',
    ifelse(Pm$Counts > 0, 'yes',
           'Mid Change'))
Pm$prevalence <- as.factor(Pm$prevalence)
levels(Pm$prevalence)
levels(Pm$prevalence) <- list("Yes"="yes", "No"="Mid Change")
Pm %>% 
  count(prevalence) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status

Pm %>%
  group_by(BRD, prevalence) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n))  ##Prevalence between healthy and sick

# Mycoplasma bovis
ASV <- t(NewASVTable)
list <- c("ASV13707", "ASV13464", "ASV2171", "ASV15117", "ASV10207")
Mb <- t(subset(ASV, rownames(ASV) %in% list))
str(Mb)
Mb <- merge(metadata, Mb, by.x = 0, by.y = 0)
Mb$ASV13707_P <- ifelse(Mb$ASV13707 < 0, 'No',
                        ifelse(Mb$ASV13707 > 0, 'yes',
                               'Mid Change'))
Mb$ASV13707_P <- as.factor(Mb$ASV13707_P)
levels(Mb$ASV13707_P)
levels(Mb$ASV13707_P) <- list("Yes"="yes", "No"="Mid Change")
Mb %>% 
  count(ASV13707_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
Mb %>% 
  count(BRD) 
Mb %>%
  group_by(BRD, ASV13707_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n)) ##Prevalence between healthy and sick


Mb$ASV13464_P <- ifelse(Mb$ASV13464 < 0, 'No',
                        ifelse(Mb$ASV13464 > 0, 'yes',
                               'Mid Change'))
Mb$ASV13464_P <- as.factor(Mb$ASV13464_P)
levels(Mb$ASV13464_P)
levels(Mb$ASV13464_P) <- list("Yes"="yes", "No"="Mid Change")
Mb %>% 
  count(ASV13464_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
Mb %>% 
  count(BRD) 
Mb %>%
  group_by(BRD, ASV13464_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n))##Prevalence between healthy and sick

Mb$ASV2171_P <- ifelse(Mb$ASV2171 < 0, 'No',
                        ifelse(Mb$ASV2171 > 0, 'yes',
                               'Mid Change'))
Mb$ASV2171_P <- as.factor(Mb$ASV2171_P)
levels(Mb$ASV2171_P)
levels(Mb$ASV2171_P) <- list("Yes"="yes", "No"="Mid Change")
Mb %>% 
  count(ASV2171_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
Mb %>% 
  count(BRD) 
Mb %>%
  group_by(BRD, ASV2171_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n)) ##Prevalence between healthy and sick

Mb$ASV15117_P <- ifelse(Mb$ASV15117 < 0, 'No',
                       ifelse(Mb$ASV15117 > 0, 'yes',
                              'Mid Change'))
Mb$ASV15117_P <- as.factor(Mb$ASV15117_P)
levels(Mb$ASV15117_P)
levels(Mb$ASV15117_P) <- list("Yes"="yes", "No"="Mid Change")
Mb %>% 
  count(ASV15117_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
Mb %>% 
  count(BRD) 
Mb %>%
  group_by(BRD, ASV15117_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n)) ##Prevalence between healthy and sick

Mb$ASV10207_P <- ifelse(Mb$ASV10207 < 0, 'No',
                        ifelse(Mb$ASV10207 > 0, 'yes',
                               'Mid Change'))
Mb$ASV10207_P <- as.factor(Mb$ASV10207_P)
levels(Mb$ASV10207_P)
levels(Mb$ASV10207_P) <- list("Yes"="yes", "No"="Mid Change")
Mb %>% 
  count(ASV10207_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
Mb %>% 
...
list1 <- c("ASV13647")
Mh <- t(subset(ASV, rownames(ASV) %in% list1))
Mh <- merge(metadata, Mh, by.x = 0, by.y = 0)

Mh$ASV13647_P <- ifelse(Mh$ASV13647 < 0, 'No',
                        ifelse(Mh$ASV13647 > 0, 'yes',
                               'Mid Change'))
Mh$ASV13647_P <- as.factor(Mh$ASV13647_P)
levels(Mh$ASV13647_P)
levels(Mh$ASV13647_P) <- list("Yes"="yes", "No"="Mid Change")
Mh %>% 
  count(ASV13647_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
Mh %>% 
  count(BRD) 
Mh %>%
  group_by(BRD, ASV13647_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n))##Prevalence between healthy and sick


### Count prevalence for Genus
ASV <- t(NewASVTable)
ASVtx <- NewTax
ASVtx$Genus <- factor(ASVtx$Genus)

#Genus histophilus
HS <- subset(ASVtx, subset=Genus %in% c("Histophilus"))
list <- rownames(HS)
HS <- t(subset(ASV, rownames(ASV) %in% list))
str(HS)
HS <- merge(metadata, HS, by.x = 0, by.y = 0)
str(HS)
HS$ASV1871_P <- ifelse(HS$ASV1871 < 0, 'No',
                        ifelse(HS$ASV1871 > 0, 'yes',
                               'Mid Change'))
HS$ASV1871_P <- factor(HS$ASV1871_P)
levels(HS$ASV1871_P) <- list("Yes"="yes", "No"="Mid Change")
HS %>% 
  count(ASV1871_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
HS %>% 
  count(BRD) 
HS %>%
  group_by(BRD, ASV1871_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n))##Prevalence between healthy and sick

#GENUS PASTEURELLA
PM <- subset(ASVtx, subset=Genus %in% c("Pasteurella"))
list <- rownames(PM)
PM <- t(subset(ASV, rownames(ASV) %in% list))
str(PM)
PM <- merge(metadata, PM, by.x = 0, by.y = 0)
str(PM)
PM$ASV8671_P <- ifelse(PM$ASV8671 < 0, 'No',
                       ifelse(PM$ASV8671 > 0, 'yes',
                              'Mid Change'))
PM$ASV8671_P <- factor(PM$ASV8671_P)
levels(PM$ASV8671_P) <- list("Yes"="yes", "No"="Mid Change")
PM %>% 
  count(ASV8671_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
PM %>% 
  count(BRD) 
PM %>%
  group_by(BRD, ASV8671_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n))##Prevalence between healthy and sick

#GENUS MANNHEIMIA
MH <- subset(ASVtx, subset=Genus %in% c("Mannheimia"))
list <- rownames(MH)
MH <- t(subset(ASV, rownames(ASV) %in% list))
str(MH)
MH <- merge(metadata, MH, by.x = 0, by.y = 0)
str(MH)
MH$ASV11615_P <- ifelse(MH$ASV11615  < 0, 'No',
                       ifelse(MH$ASV11615  > 0, 'yes',
                              'Mid Change'))
MH$ASV11615_P <- factor(MH$ASV11615_P)
levels(MH$ASV11615_P) <- list("Yes"="yes", "No"="Mid Change")
MH %>% 
  count(ASV11615_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
MH %>% 
  count(BRD) 
MH %>%
  group_by(BRD, ASV11615_P) %>%
  tally  %>%
  group_by(BRD) %>%
  mutate(pct=(100*n)/sum(n))##Prevalence between healthy and sick

#GENUS MYCOPLASMA 
MB <- subset(ASVtx, subset=Genus %in% c("Mycoplasma"))
list <- rownames(MB)
MB <- t(subset(ASV, rownames(ASV) %in% list))
str(MB)
MB <- merge(metadata, MB, by.x = 0, by.y = 0)
str(MB)
MB$ASV11222_P <- ifelse(MB$ASV11222  < 0, 'No',
                        ifelse(MB$ASV11222  > 0, 'yes',
                               'Mid Change'))
MB$ASV11222_P <- factor(MB$ASV11222_P)
levels(MB$ASV11222_P) <- list("Yes"="yes", "No"="Mid Change")
MB %>% 
  count(ASV11222_P) %>%
  mutate(pct=(100*n)/sum(n)) ### Prevalence regardless healthy status
MB %>% 
